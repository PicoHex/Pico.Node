...
switch (mode)
{
    ...
    default:
        Console.WriteLine(
            "Modes: tcp-throughput | tcp-throughput-sweep | tcp-throughput-parallel | tcp-churn | udp-rate | udp-rate-parallel"
        );
...
static async Task RunUdpRateParallelAsync(Dictionary<string, string> options)
{
    ...
    PrintMetrics("udp-rate-parallel", totalDatagrams, totalBytes, stopwatch.Elapsed, "datagrams/s");
}
